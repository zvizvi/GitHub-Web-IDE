...
#open-in-web-ide .btn.BtnGroup-item {
  height: 32px;
  min-width: 32px;
}
#gh-repo-size.btn.BtnGroup-item {
  height: 32px;
  min-width: 32px;
}`;
...
async function updateRepoSize(elementId) {
    const element = document.getElementById(elementId);
    if(!element) return;
    
    // Reset text if cached or loading
    element.textContent = '...';

    if (repoSizeCache[repoUrlPath]) {
         element.textContent = repoSizeCache[repoUrlPath];
         return;
    }

    try {
        const response = await fetch(`https://api.github.com/repos/${repoUrlPath}`);
        if(response.ok) {
            const data = await response.json();
            // API size is in KB
            const sizeStr = formatBytes(data.size * 1024); 
            element.textContent = sizeStr;
            repoSizeCache[repoUrlPath] = sizeStr;
        } else {
            element.textContent = 'N/A';
        }
    } catch(e) {
        console.error('Error fetching repo size:', e);
        element.textContent = '?';
    }
}
...
function addGitHubSelectMenu () {
  ...
  // New HTML Structure using BtnGroup
  const githubHtml = `
  <div id="open-in-web-ide" class="BtnGroup d-flex">
    <details class="details-overlay details-reset position-relative BtnGroup-parent">
      <summary role="button" type="button" class="btn BtnGroup-item text-center">
    ...
    <button id="gh-repo-size" class="btn BtnGroup-item tooltipped tooltipped-s" aria-label="Repository Size" style="cursor: default;" type="button">
        ...
    </button>
  </div>`;

  const tempWrapper = document.createElement('div');
  tempWrapper.innerHTML = githubHtml;
  const newContent = tempWrapper.firstElementChild;

  menuElement.appendChild(newContent);
