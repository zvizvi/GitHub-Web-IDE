  ...
  switch (platform) {
    case 'github': {
      const itemWithBorderOnTop = ideWebsitesList.find( // first item only
        (item) => (item.name === 'cloneInVSCode' || item.name === 'openInVSCode') && filterItems(item));
...
function addGithubSelectMenu () {
  ...
  <div>
    ...
    `<li class="${item.class || ''}" data-toggle-for="open-in-web-ide"><a href="${item.baseurl}${item.type === 'repoName' ? repoUrlPath : repoUrlFullPath}" class="dropdown-item" ${(!options || options.openInNewTab) && item.openInNewTab ? 'target="_blank"' : ''} rel="noopener noreferrer">
        ${item.icon}
        ${item.title}
      </a></li>`).join('')}
    </ul>
